...
async function parseSessionFile(filePath: string, signal?: AbortSignal): Promise<ParsedSession | null> {
	...
	try {
		for await (const line of rl) {
			...
			const tokens = extractTotalTokens(usage);
			if (tokens > 0) {
				totalTokens += tokens;
				tokensByModel.set(mk, (tokensByModel.get(mk) ?? 0) + tokens);
			}
		}
	} finally {
		rl.close();
	...
	return { filePath, startedAt, dayKeyLocal, modelsUsed, totalCost, costByModel, totalTokens, tokensByModel };
}
...
function addSessionToRange(range: RangeAgg, session: ParsedSession): void {
	...
	range.totalTokens += session.totalTokens;
	day.sessions += 1;
	...
	// Tokens-per-model
	for (const [mk, tokens] of session.tokensByModel.entries()) {
		day.tokensByModel.set(mk, (day.tokensByModel.get(mk) ?? 0) + tokens);
		range.modelTokens.set(mk, (range.modelTokens.get(mk) ?? 0) + tokens);
	}
}
...
function renderModelTable(range: RangeAgg, maxRows = 8): string[] {
	...
	const lines: string[] = [];
	lines.push(
		`${padRight("model", modelWidth)}  ${padLeft("sessions", 8)}  ${padLeft("tokens", 10)}  ${padLeft("cost", 10)}  ${padLeft("share", 6)}`,
	);
	lines.push(`${"-".repeat(modelWidth)}  ${"-".repeat(8)}  ${"-".repeat(10)}  ${"-".repeat(10)}  ${"-".repeat(6)}`);

	for (const r of rows) {
		const sessions = range.modelSessions.get(r.key) ?? 0;
		const tokens = range.modelTokens.get(r.key) ?? 0;
		...
		lines.push(
			`${padRight(r.key.slice(0, modelWidth), modelWidth)}  ${padLeft(String(sessions), 8)}  ${padLeft(formatTokens(tokens), 10)}  ${padLeft(formatUsd(cost), 10)}  ${padLeft(share, 6)}`,
		);
	}
...
function rangeSummary(range: RangeAgg, days: number): string {
	...
	const tokensPart = range.totalTokens > 0 ? ` · ${formatTokens(range.totalTokens)} tokens` : "";
	return `Last ${days} days: ${range.sessions} sessions${tokensPart} · ${costPart}`;
}
